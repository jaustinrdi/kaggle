...
print("%s" % time.time() + " : " + "Loading data...")
...
forest = ensemble.ExtraTreesClassifier(n_estimators=350, n_jobs=-1)

print("%s" % time.time() + " : " + "Fitting...")
forest.fit(X_train, y)

print("%s" % time.time() + " : " + "Predicting...")
d = pd.DataFrame(0, index=np.arange(len(test['Id'])), columns=['Id', 'Cover_Type'])
d['Id'] = test['Id']
Y_test = forest.predict(X_test)
d['Cover_Type'] = Y_test

print("%s" % time.time() + " : " + "Writing to file...")
d.to_csv('files/submission_' + datetime.now().strftime("%H%M%S") + ".csv", index=False)

# look at feature importances
# pd.DataFrame(forest.feature_importances_, index=X_train.columns).sort([0], ascending=False)[:10]

print("%s" % time.time() + " : " + "done")
